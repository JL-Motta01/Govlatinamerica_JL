...
for entry in entries:
    ...
    print('with find before if', authors,sep=' ### ')
    # authors = entry.select_one('a').get_text()
    # print('with selector before if', authors, sep=' ### ')
    time = entry.select_one('strong').get_text()
    # time = entry.find('strong').text
    if time == 'TITLE':
        continue  # skip this entry
    authors = bs.find('a').text
    print('with find after if', authors,sep=' ### ')
    authors = entry.select_one('a').get_text()
    print('with selector after if', authors, sep=' ### ')
    # Grab a list of the links (may be of size 0-2 depending on the contents).
    links = [link.get('href') for link in entry.select('a')]
 
    if entry.select_one('a').get_text() != entry.find('a').text:
        print('difference!')
