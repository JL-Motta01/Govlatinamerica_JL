...
def coleta_compromissos(ag,origem):
    ...
    for dia in agenda(ag):
        ...
        db = TinyDB(f"{dir_json}/agenda_GSI.json", indent=4, ensure_ascii=False)
        User = Query()
        try:
...
def acesso_ministerios ():  
    lista_ministerios = ["https://www.gov.br/gsi/pt-br/acesso-a-informacao/institucional/agendas-do-gsi"]
    lista_agenda = []
