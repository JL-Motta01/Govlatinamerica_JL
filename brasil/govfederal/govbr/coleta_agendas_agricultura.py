...
def coleta_compromissos(ag,origem):
    ...
    for dia in agenda(ag):
        ...
        db = TinyDB(f"{dir_json}/agenda_agricultura.json", indent=4, ensure_ascii=False)
        User = Query()
        try:
...
def acesso_ministerios ():
    lista_ministerios = ["https://www.gov.br/agricultura/pt-br/acesso-a-informacao/agendas"]
    lista_agenda = []
